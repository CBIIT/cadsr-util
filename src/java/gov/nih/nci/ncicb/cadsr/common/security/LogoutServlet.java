...
public class LogoutServlet extends HttpServlet {
	...
	protected void doPost(
		...
		synchronized (SessionUtils.sessionObjectCache) {
			...
			if ((session != null) && isLoggedIn(request)) {
				...
				forwardUrl =forwardUrl + "?" + CaDSRConstants.PREVIOUS_SESSION_ID + "=" +
				session.getId();
				session.invalidate();
			}
	...
	throws ServiceLocatorException {
		ApplicationServiceLocator appServiceLocator = (ApplicationServiceLocator) sc.getAttribute(ApplicationServiceLocator.APPLICATION_SERVICE_LOCATOR_CLASS_KEY);
